...
passenger_cout: No tiene sentido que el mínimo sea 0 ya que por defecto el número de pasajeros mínimo es 1.
trip_time_in_secs: No tiene sentido que el mínimo sea 0 segudos, ya que bajando la bandera de pasajero el mínimo siempre seria mayor que 0.
trip_distance: Salvo caso muy raro, no debería tener mínimo de 0 de distancia.
pickup_longitude: El mínimo de longitud del globo terráqueo seria -90, no existe el valor indicado de -2014.3392. Tambien se descartaria por el bounding box de NY.
pickup_latitude: En este caso no tiene sentido ni el mínimo ni el máximo.
dropoff_longitude: El mínimo no tiene sentido por el mismo motivo que en los dos casos anteriores.
dropoff_latitude: El mínimo y el máximo se exceden de las coordenadas máximas de la tierra.
...
# -74.25909,40.477399,-73.700181,40.916178
nw <- list(lat = 40.916178, lon = -74.25909)
se <- list(lat = 40.477399, lon = -73.700181)
```
...
col <- colnames(df, do.NULL = FALSE, prefix="row")
for (i in 0:ncol(df)){
  print(col[i])
  ok <- complete.cases(df[i])
  print(table(ok))
  percentageNAN = (sum(!ok)/(sum(ok)+sum(!ok)))
  print(percentageNAN)
...
Tiempo medio de una carrera es de 11 minutos y 6 segundos, con un mmínimo de 1 segundo y un máximo de 2 horas y 49 minutos.
Distancia media de una carrera es de 2.823 millas.
...
Cogiendo la distancia media y el tiempo medio obtemos que conducen de velocidad media a 24,54 km/h.

2.823/(666/3600)=millas/hora -> * 1.60934 -> km/h
...
La media de pasajeros por viajes es de 1.647 pasajeros, generalmente van 2 o más.
...
La mayoría de recogidas se realizan en las mismas zonas, Manhattan, mientras que las bajadas se ven distribuidas por todo nueva york, estoy puede ser debido a que los ciudadanos usan el taxi para volver a sus domicilios despues de trabajar, habría que hacer otra gráfica con las horas de bajada y subida de pasajeros para contrastar esta hipótesis.
No obstante aqui se puede apreciar datos de cuando se suele coger el taxi y cuando se suele dejar, ambos despues del medio dia, que puede coincidir con el final de media jornada de trabajo.

pickup_datetime               dropoff_datetime           
Mean   :2013-01-14 14:22:40   Mean   :2013-01-14 14:33:46
...
Se ve incrementado la bajada de taxis a primera hora del día, para empezar la jornada y luego entre las 17.30 y las 18.30 a la salida del trabajo y colegio.
...
```{r}
#-73.8352, 40.6195, -73.7401, 40.6659.
jfk_nw <- list(lat = 40.660052, lon = -73.821412)
jfk_se <- list(lat = 40.631317, lon = -73.758051)
...
```{r}
print("JFK_PICKUPS")
print(nrow(jfk_pickups))

print("JFK_DROPOFFS")
print(nrow(jfk_dropoffs))
```
...
Si el número de rows en la base de datos es de 443384 y el numero de salidas y llegadas del aeropuerto es de 11631, el porcentaje es de 2.62 %
...
Las salidas hacia el aeropuerto parte principalmente de la zona de Manhatta y algunas de Broklyn, mientras que desde el aeropuerto todas van directamente a Manhattan, zona de Upper Easy Side.
...
Los viajes tardan en torno a los 27 minutos.
...
ts_dropoff <- trips_manh_jfk %>% 
                            mutate(hour = dropoff_by_hour) %>% 
                            group_by(hour) %>% 
                            summarise(q5 = quantile(trip_time_in_secs/60, prob=0.05, na.rm = TRUE),
                                      q25 = quantile(trip_time_in_secs/60, prob=0.25, na.rm = TRUE), 
                                      q50 = median(trip_time_in_secs/60), 
                                      q75 = quantile(trip_time_in_secs/60, prob=0.75, na.rm = TRUE),
                                      q95 = quantile(trip_time_in_secs/60, prob=0.95, na.rm = TRUE))
...
En los dos picos que se muestran es donde habría más tráfico, a las 8 y sobre las 16.30
...
Sobre las 11.30, los margenes tanto superior como inferior son muy amplios.
...
La gráfica de día esta contenido en la gráfica de noche, mientras que en la de noche se ve un incremento de toma de taxis en la zona de Manhattan.
